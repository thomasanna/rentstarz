...
class User_View_Helper_SmartmoveApi extends Zend_View_Helper_Abstract
...
//exit;
    ...
    public function DeclineApplicationRenterStatus($email,$applicationId){
   ...
   public function getcountrystatecity(){
    ...
    $countrystatecitySelect   =  $propertycountrtyTable->select()
                        ->setIntegrityCheck(false)
                        ->from(array('country'=>'engine4_property_countries',))
                        ->joinLeft(array('state'=>'engine4_property_states',),'country.country_id=state.country_id',array('state_id','prty_state'))
                        ->joinLeft(array('city'=>'engine4_property_city',),'city.state_id=state.state_id',array('city_id','prty_city'))
                        ->where('country.country_id =?' ,  1)
                        ->group('city.city_id');
    $countrystatecityData    =  $propertycountrtyTable->fetchAll($countrystatecitySelect);
       foreach($countrystatecityData as $data){
         $locationInfo       =   array(
                                'value'  =>  $data['city_id'].','.$data['state_id'].','.$data['country_id'],
                                'location'=>  $data['prty_city'].'  ,  '.$data['prty_state'].'  ,  '.$data['prty_country']
                                 );
         $tmpLocationInfo[]  =   $locationInfo;
       }
